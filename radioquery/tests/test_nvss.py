...
from survey_configs.nvss import NvssQuery
...
def test_rquery_directories():
    """Ensure required directory structure exists."""
    base_dir = os.path.expanduser("~/RQUERY")
    nvss_path = os.path.join(base_dir, "NVSS")
    assert os.path.isdir(nvss_path)
...
def test_first_download_full(test_coord): 
    ...
    nq = NvssQuery(coord=test_coord, download_path=full_path, size_arcmin=15.0,overwrite=True)
    ...
    dataset_path = os.path.join(test_dir, "datasets", "NVSS_J105007.27+304037.52.fits")
